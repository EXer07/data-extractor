...
class PowerBiScraper:
    ...
    def _switch_if_iframe(self):
        ...
        # Power BI itself creates sandbox iframes for visuals. If the table is
        # already in the current document, switching to the first iframe moves
        # away from the report and makes the table impossible to find.
        if self._driver.find_elements(By.CSS_SELECTOR, ".tableEx"):
            logger.debug("Table found in current document; not switching iframe")
